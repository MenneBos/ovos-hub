...
# Install STT plugins or others Python libraries via pip command when a setup.py exists
if [[ -n "$file" ]]; then
    if ! diff -q -B <(grep -vE '^\s*(#|$)' "$file") <(grep -vE '^\s*(#|$)' "$listener_list_state" 2>/dev/null) &>/dev/null; then
        ...
        uv pip install -r "$listener_list"
...
echo "Starting mycroft-classic-listener"
exec mycroft-classic-listener